...
export const TransactionModal: React.FC<TransactionModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-0 md:p-4">
      <div className="bg-white rounded-none md:rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col h-full md:h-auto md:max-h-[95vh]">
        
        <div className="bg-slate-900 pt-4 px-4 pb-0 text-white flex flex-col gap-4 shrink-0">
          ...
          {!editingTransaction && (
            <div className="flex gap-1">
               <button 
                  ...
                  className={`flex-1 py-3 text-sm font-bold rounded-t-lg transition-colors flex items-center justify-center gap-2 ${tab === 'ENTRY' ? 'bg-white text-slate-900' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
               >
                  <DollarSign size={16}/> Registrar Pago
               </button>
               <button 
                  onClick={() => { setTab('EXIT'); setSourceType('TREASURY'); }} 
                  className={`flex-1 py-3 text-sm font-bold rounded-t-lg transition-colors flex items-center justify-center gap-2 ${tab === 'EXIT' ? 'bg-white text-slate-900' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
               >
