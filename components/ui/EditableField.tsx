...
export const EditableField: React.FC<EditableFieldProps> = ({ 
  ...
  const inputBaseClasses = "w-full bg-white border-b-2 border-blue-500 px-1 py-0.5 text-sm text-slate-800 outline-none transition-all";
  ...
  if (readOnly) {
    return (
      <div className={`flex flex-col group ${className}`}>
        ...
        <div className="text-sm font-bold text-slate-700 px-1 py-0.5 min-h-[24px] flex items-center">
    ...
    <div className={`flex flex-col group ${className}`}>
      ...
      {isEditing ? (
        ...
        <div 
          ...
          className="text-sm font-bold text-slate-700 cursor-pointer hover:bg-slate-100 hover:text-blue-700 px-1 -ml-1 py-0.5 rounded transition-colors flex items-center gap-2 min-h-[24px]"
          title="Clic para editar"
        >
          <span className="truncate">
             {isCurrency && value ? `$ ${formatNumberWithDots(value)}` : (value || <span className="text-slate-300 font-normal italic">{placeholder}</span>)}
          </span>
          <Edit2 size={10} className="opacity-0 group-hover:opacity-40 text-blue-400" />
        </div>
      )}
    </div>
  );
